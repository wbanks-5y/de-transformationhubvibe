...
export const useHomeCockpitAggregates = () => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: cockpitTypes, error: cockpitTypesError } = await supabase
        .from('cockpit_types')
      ...
      for (const cockpitType of cockpitTypes || []) {
        ...
        if (!kpis || kpis.length === 0) {
          console.log('⚠️ No KPIs found for cockpit:', cockpitType.display_name);
          continue;
        }
        ...
        for (const kpi of kpis) {
          ...
          if (kpi.kpi_data_type === 'single') {
            ...
            const { data: timeBasedValue } = await supabase
          ...
          if (currentValue !== null && targetValue !== null && targetValue > 0) {
            ...
            console.log('⚠️ Skipping KPI (missing data):', kpi.display_name, {
