  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        console.log(
          `[${correlationId}] POST: RESEND_API_KEY || RESEND_API_KEY_V2 ${resendApiKey ? "FOUND" : "NOT FOUND"} - ${resendApiKey ? "Will send email via Resend" : "Skipping custom email"}`,
        );
    ...
    if (req.method === "DELETE") {
      ...
      try {
        ...
        const user = authData?.users?.find(
          (u) => (u.email || "").toLowerCase() === emailNormalized && !u.email_confirmed_at,
        );
        if (user) {
        ...
        const { error: tokenDeleteError } = await managementClient
