...
serve(async (req) => {
  ...
  try {
    ...
    const MANAGEMENT_KEY =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZnYmlscHp1bml1cXJwZXRuYmd6Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1ODgwNDk3MywiZXhwIjoyMDc0MzgwOTczfQ.jgEPWkNgJmMo6VG-LWZpcVrq9E-j8n472RN9atnh7Z0";
    ...
    const { data: tokenRow, error: tokenErr } = (await managementClient
      .from("invitation_tokens")
      .select(
        "id, token, email, organization_id, organization_slug, " +
          "organization_supabase_url, organization_supabase_anon_key, expires_at, used_at",
      )
    ...
    const { data: orgRow, error: orgErr } = await managementClient
      .from("organizations")
      .select("id, slug, supabase_service_role_key")
    ...
    const orgUrl = tokenRow.organization_supabase_url?.startsWith("http")
      ? tokenRow.organization_supabase_url
      : `https://${tokenRow.organization_supabase_url}`;
    ...
    const { data: insertData, error: userOrgError } = await managementClient
