...
-- Create an optimized function to calculate all cockpit aggregates in a single query
...
BEGIN
  ...
  cockpit_aggregates AS (
    SELECT 
      ...
      color_class,
      COUNT(*) as total_kpis,
      COUNT(*) FILTER (WHERE is_active = true) as active_kpis,
      -- Calculate weighted achievement percentage
      CASE 
        WHEN SUM(weight) FILTER (WHERE current_value IS NOT NULL AND target_value IS NOT NULL AND target_value > 0) > 0 THEN
          SUM(
            CASE 
              WHEN current_value IS NOT NULL AND target_value IS NOT NULL AND target_value > 0 THEN
                CASE 
                  WHEN trend_direction = 'lower_is_better' THEN
                    LEAST((target_value / NULLIF(current_value, 0)) * 100 * weight, 100 * weight)
                  ELSE
                    LEAST((current_value / target_value) * 100 * weight, 100 * weight)
                END
              ELSE 0
            END
          ) / SUM(weight) FILTER (WHERE current_value IS NOT NULL AND target_value IS NOT NULL AND target_value > 0)
        ELSE 0
      END as performance_percentage
    FROM cockpit_kpi_data
    WHERE cockpit_type_id IS NOT NULL
    GROUP BY cockpit_type_id, cockpit_name, display_name, icon, color_class
  )
  SELECT 
    ca.cockpit_type_id,
    ca.cockpit_name,
...
CREATE INDEX IF NOT EXISTS idx_cockpit_kpi_values_kpi_recorded 
ON public.cockpit_kpi_values(kpi_id, recorded_at DESC);

CREATE INDEX IF NOT EXISTS idx_cockpit_kpi_time_based_kpi_period 
ON public.cockpit_kpi_time_based(kpi_id, period_start DESC);

CREATE INDEX IF NOT EXISTS idx_cockpit_kpi_targets_kpi_active_created 
ON public.cockpit_kpi_targets(kpi_id, is_active, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_cockpit_types_active 
ON public.cockpit_types(is_active) WHERE is_active = true;

CREATE INDEX IF NOT EXISTS idx_cockpit_kpis_cockpit_active 
ON public.cockpit_kpis(cockpit_type_id, is_active) WHERE is_active = true;
