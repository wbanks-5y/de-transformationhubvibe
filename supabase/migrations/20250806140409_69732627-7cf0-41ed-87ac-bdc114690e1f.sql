...
-- Function to encrypt API keys using a fixed key for now (will be improved with proper key management)
...
-- Phase 2: Clean up RLS policies - Remove ALL existing policies first
...
DROP POLICY IF EXISTS "Allow admin users to manage profiles" ON public.profiles;
...
-- Now create clean, single policies
...
ON public.profiles
FOR INSERT
WITH CHECK (is_admin_secure() OR auth.uid() = id);
...
DROP TRIGGER IF EXISTS update_encrypted_api_keys_updated_at ON public.encrypted_api_keys;
