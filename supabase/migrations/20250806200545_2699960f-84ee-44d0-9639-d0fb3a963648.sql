...
-- Drop and recreate the view without SECURITY DEFINER property using correct column names
...
SELECT 
  ...
  COALESCE(
    CASE 
      WHEN COUNT(sra.id) > 0 AND SUM(sra.allocated_amount) > 0 THEN
        (SUM(COALESCE(sra.utilized_amount, 0)) / SUM(sra.allocated_amount)) * 100
      ELSE 0
    END, 0
