...
-- Fix the ambiguous column reference issue in get_home_cockpit_aggregates function
...
BEGIN
  ...
  cockpit_aggregates AS (
    ...
    GROUP BY ckd.cockpit_type_id, ckd.cockpit_name, ckd.display_name, ckd.icon, ckd.color_class
  )
  SELECT 
    ca.cockpit_type_id,
    ca.cockpit_name,
